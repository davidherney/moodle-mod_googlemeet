...
$string['recordingfilter_help'] = 'Enter the exact text that recording filenames must begin with. The text must be followed by a separator such as a space, hyphen or parenthesis. When set, only new recordings that match this prefix are synchronised; recordings already synchronised are kept as long as they remain findable in Drive. Leave empty to use the original Google Calendar event name.';
$string['autosynchours'] = 'Auto-sync hours after session end';
$string['autosynchours_help'] = 'Hours to wait after a scheduled session ends before automatically syncing recordings from Google Drive. Set to 0 to disable auto-sync (teachers still can sync manually). Each session is only attempted once; if Google Drive has not published the recording yet, the teacher will need to sync manually.';
